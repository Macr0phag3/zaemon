...
class CLI_Parser:
    ...
    def _parse_exec(self):
        exec_parser = self.subparsers.add_parser("exec", help="在容器中执行命令")
        exec_parser.add_argument("id", help="容器 ID")
        exec_parser.add_argument(
            "cmd", nargs=argparse.REMAINDER, help="要运行的命令及参数"
        )

    def _parse_stop(self):
        exec_parser = self.subparsers.add_parser("stop", help="停止一个容器")
        exec_parser.add_argument("id", help="容器 ID")

    def _stop(self):
        container = find_container_by_prefix(get_container_info(), self.args.id)
        if container["status"] != "running":
            output(f"container {self.args.id} is already stopped", "warning")
            return

        proc_info = get_container_proc_info(container)
        os.kill(proc_info["pid"], 9)

    def _exec(self):
        container = find_container_by_prefix(get_container_info(), self.args.id)
        if container["status"] != "running":
            output(f"container {self.args.id} is not running", "warning")
            return
        ...
        for nstype in ["ipc", "net", "pid", "uts", "mnt"]:
            ...
            try:
                with open(ns_path, "r") as ns_file:
                    ...
                    output(f"Switched to {nstype} namespace of PID {pid}", "debug")
            except PermissionError:
                output(
                    "Permission denied. Are you running the script as root?", "warning"
                )
            except Exception as e:
                output(f"Failed to switch namespace: {e}", "warning")

        output(f"执行命令 {self.args.cmd}", "debug")
        os.execvp(self.args.cmd[0], self.args.cmd)
